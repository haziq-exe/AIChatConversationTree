(() => {
  ...
  function attachRuntimeMessageListener() {
    chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
      ...
      if (message.type === "CGBT_TOGGLE_PANEL_FROM_ACTION") {
        if (!state.hasConversation) {
          sendResponse({ ok: true, opened: false, reason: "no-conversation" });
          return false;
        }

        setPanelOpen(!state.panelOpen, true);
        sendResponse({ ok: true, opened: state.panelOpen });
        return false;
      }
  ...
  function findAssistantResponseNodeId(userNodeId, mapping) {
    ...
    while (queue.length > 0) {
      ...
      if (role === "assistant" && shouldRenderNode(node, role, text)) {
        if (isLikelyToolCallPayloadText(text)) {
          continue;
        }
        return nodeId;
      }
  ...
  function isLikelyToolCallPayloadText(text) {
    const normalized = normalizeWhitespace(text);
  ...
  function getPreviewPair(nodeId, mapping) {
    const turn = state.visibleNodes.find((item) => item.nodeId === nodeId);
    if (turn) {
      const messageText = turn.userText || "(Message has no text content.)";
      const responseText = turn.assistantText
        ? turn.assistantText
  ...
  async function replayTurnPathThroughUi(turnPath) {
    ...
    debugNav("replay:start", {
      provider: state.provider,
      pathLength: turnPath.length,
      targetNodeId: turnPath[turnPath.length - 1] ? turnPath[turnPath.length - 1].nodeId : null
    });
    ...
    for (let index = 0; index < turnPath.length - 1; index += 1) {
      ...
      if (children.length <= 1) {
        debugNav("replay:skip-single-child", { parent: parentTurn.nodeId });
        continue;
      }

      const targetIndex = children.indexOf(childTurn.nodeId);
      if (targetIndex < 0) {
        debugNav("replay:skip-target-index-missing", { parent: parentTurn.nodeId, child: childTurn.nodeId });
        continue;
      }
      debugNav("replay:step", {
        step: index,
        parent: parentTurn.nodeId,
        child: childTurn.nodeId,
        targetIndex,
      ...
      if (!anchorSelection || !anchorSelection.element) {
        debugNav("replay:anchor-missing", { parent: parentTurn.nodeId });
        return {
          ok: false,
      ...
      const expectedVariantTotal = children.length;
      const switched = await setVariantToRevealChild(
        anchor,
        snippet,
        role,
        targetIndex,
        childTurn,
        expectedVariantTotal
      );
      if (!switched) {
        debugNav("replay:switch-failed", { parent: parentTurn.nodeId, child: childTurn.nodeId, targetIndex });
        return {
          ok: false,
      ...
      if (!childVisible) {
        const stillAtExpectedVariant = isVariantIndexNearAnchor(
          anchor,
          snippet,
          role,
          targetIndex,
        ...
        debugNav("replay:child-not-visible", {
          parent: parentTurn.nodeId,
          child: childTurn.nodeId,
          targetIndex,
          stillAtExpectedVariant
        });
        if (stillAtExpectedVariant) {
          await sleep(220);
          continue;
        }
        return {
    ...
    debugNav("replay:target-visibility", {
      target: targetTurn ? targetTurn.nodeId : null,
      found: Boolean(targetElement)
    });
    if (targetElement) {
  ...
  async function waitForTurnVisible(turn, timeoutMs) {
    ...
    const userSnippet = createDomSearchSnippet(turn.userText || turn.userSnippet || "");
    if (userSnippet) {
      const userElement = await waitForMessageElement(userSnippet, "user", timeoutMs, { allowScrollScan: false });
      if (userElement) {
        debugNav("visibility:user-match", {
          nodeId: turn.nodeId,
          snippet: userSnippet.slice(0, 100)
        });
        return userElement;
      }
      debugNav("visibility:user-miss", {
        nodeId: turn.nodeId,
        snippet: userSnippet.slice(0, 100)
      });
    }

    const assistantSnippet = createDomSearchSnippet(turn.assistantText || turn.assistantSnippet || "");
    if (!assistantSnippet) {
      debugNav("visibility:assistant-snippet-empty", { nodeId: turn.nodeId });
      return null;
    }

    const assistantTimeout = Math.max(500, Math.floor(timeoutMs * 0.75));
    const assistantElement = await waitForMessageElement(
      assistantSnippet,
      "assistant",
      assistantTimeout,
      { allowScrollScan: false }
    );
    debugNav("visibility:assistant", {
      nodeId: turn.nodeId,
      found: Boolean(assistantElement),
      snippet: assistantSnippet.slice(0, 100)
    });
    return assistantElement;
  }

  async function setVariantToRevealChild(anchor, snippet, role, preferredIndex, childTurn, expectedVariantTotal) {
    const totalVariants = getVariantTotalNearAnchor(anchor, snippet, role, expectedVariantTotal);
    if (!Number.isFinite(totalVariants) || totalVariants <= 1) {
      debugNav("variant:no-variants", { snippet: snippet.slice(0, 100), totalVariants });
      return false;
    }

    const hasPreferred =
      Number.isInteger(preferredIndex) && preferredIndex >= 0 && preferredIndex < totalVariants;
    if (hasPreferred) {
      debugNav("variant:preferred", {
        preferredIndex,
        totalVariants,
        expectedVariantTotal,
        child: childTurn ? childTurn.nodeId : null
      });
      const switched = await setVariantIndexNearAnchor(
        anchor,
        snippet,
        role,
        preferredIndex,
      ...
      if (!switched) {
        debugNav("variant:preferred-switch-failed", { preferredIndex, totalVariants });
        return false;
      }

      const childVisible = await waitForTurnVisible(childTurn, 1_200);
      if (childVisible) {
        debugNav("variant:preferred-visible", { preferredIndex });
        return true;
      }

      const stillAtPreferred = isVariantIndexNearAnchor(
        anchor,
        snippet,
        role,
        preferredIndex,
      ...
      debugNav("variant:preferred-counter-check", { preferredIndex, stillAtPreferred });
      return stillAtPreferred;
    }
    ...
    for (const index of candidateIndexes) {
      const switched = await setVariantIndexNearAnchor(anchor, snippet, role, index, expectedVariantTotal);
      if (!switched) {
        continue;
      }
      const childVisible = await waitForTurnVisible(childTurn, 900);
      if (childVisible) {
        debugNav("variant:fallback-visible", { index });
        return true;
      }
      debugNav("variant:fallback-not-visible", { index });
      await sleep(120);
    }

    debugNav("variant:failed-all-candidates", { totalVariants });
    return false;
  }

  function getVariantTotalNearAnchor(anchor, snippet, role, expectedVariantTotal) {
    const refreshedAnchor = findMessageElement(snippet, role) || anchor;
    const variantControls = findVariantControls(refreshedAnchor, expectedVariantTotal);
  ...
  function isVariantIndexNearAnchor(anchor, snippet, role, targetIndex, expectedVariantTotal) {
    const refreshedAnchor = findMessageElement(snippet, role) || anchor;
    const variantControls = findVariantControls(refreshedAnchor, expectedVariantTotal);
    if (!variantControls || !variantControls.counter) {
      return false;
    ...
    const boundedTarget = clamp(targetIndex, 0, parsed.total - 1);
    return parsed.current === boundedTarget;
  }
  ...
  async function setVariantIndexNearAnchor(anchor, snippet, role, targetIndex, expectedVariantTotal) {
    const maxClicks = 36;
    for (let attempt = 0; attempt < maxClicks; attempt += 1) {
      const refreshedAnchor = findMessageElement(snippet, role) || anchor;
      const variantControls = findVariantControls(refreshedAnchor, expectedVariantTotal);
      if (!variantControls) {
        debugNav("variant:set-index:no-controls", { targetIndex, attempt });
        return false;
      }

      const parsedCounter = parseVariantCounter(variantControls.counter.textContent || "");
      if (!parsedCounter || parsedCounter.total <= 1) {
        debugNav("variant:set-index:invalid-counter", {
          targetIndex,
          attempt,
          counterText: variantControls.counter ? variantControls.counter.textContent : ""
        });
        return false;
      }

      const boundedTarget = clamp(targetIndex, 0, parsedCounter.total - 1);
      debugNav("variant:set-index:state", {
        attempt,
        current: parsedCounter.current,
        total: parsedCounter.total,
        boundedTarget
      });
      ...
      if (parsedCounter.current < boundedTarget) {
        if (!variantControls.next) {
          debugNav("variant:set-index:missing-next", { attempt, boundedTarget });
          return false;
        }
        variantControls.next.click();
      } else {
        if (!variantControls.prev) {
          debugNav("variant:set-index:missing-prev", { attempt, boundedTarget });
          return false;
        }
  ...
  function findVariantControls(anchor, expectedVariantTotal) {
    const scopes = collectCandidateScopes(anchor);
    const expectedTotal =
      Number.isInteger(expectedVariantTotal) && expectedVariantTotal > 1 ? expectedVariantTotal : null;
    const candidates = [];
    ...
    for (const scope of scopes) {
      ...
      for (const counter of counters) {
        ...
        if (buttons.prev || buttons.next) {
          const counterDistance = anchor ? elementDistance(counter, anchor) : Number.POSITIVE_INFINITY;
          candidates.push({
            counter,
            prev: buttons.prev,
            next: buttons.next,
            total: parsed.total,
            totalMatch: expectedTotal !== null && parsed.total === expectedTotal,
    ...
    candidates.sort((left, right) => {
      if (left.totalMatch !== right.totalMatch) {
        return left.totalMatch ? -1 : 1;
      }
      ...
      if (expectedTotal !== null) {
        const leftTotalDelta = Math.abs(left.total - expectedTotal);
        const rightTotalDelta = Math.abs(right.total - expectedTotal);
        if (leftTotalDelta !== rightTotalDelta) {
          return leftTotalDelta - rightTotalDelta;
        }
      }
    ...
    debugNav("variant:controls-selected", {
      expectedTotal,
      selectedTotal: selected.total,
      totalMatch: selected.totalMatch,
      distance: Number.isFinite(selected.distance) ? Math.round(selected.distance) : null
    });
  ...
  function findVariantButtons(scope, counter) {
    ...
    const fallbackButtons = buttons.filter((button) => {
      const fingerprint = buttonFingerprint(button);
      if (!fingerprint) {
        return true;
      }
      return !/\b(sources?|citations?|footnotes?)\b/i.test(fingerprint);
    });
